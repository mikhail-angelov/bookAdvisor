...
import { initDatabase, closeDatabase, getDb, torrents, torrentDetails, crawlHistory } from '@/lib/db/index';
...
function parseTorrentRow($: cheerio.CheerioAPI, row: any) {
  ...
  const seeds = parseInt($row.find('td:nth-child(4)').text().trim().replace(/[^\d]/g, ''), 10) || 0;
  const leechers = parseInt($row.find('td:nth-child(5)').text().trim().replace(/[^\d]/g, ''), 10) || 0;
  const downloads = parseInt($row.find('td:nth-child(6)').text().trim().replace(/[^\d]/g, ''), 10) || 0;
...
describe('Crawler Integration Tests', () => {
  ...
  describe('Database Storage', () => {
    ...
    it('should store all torrent fields correctly', async () => {
      ...
      const results = await testDb.select().from(torrents).where(eq(torrents.topicId, '123456'));
  ...
  describe('End-to-End Crawl Test', () => {
    ...
    it('should handle empty HTML gracefully', () => {
      const emptyHTML = '<html><body><table class="vf-table vf-tor forumline forum"><tbody></tbody></table></body></html>';
      const parser = new RutrackerParser();
...
`;
    ...
    it('should parse 50 torrents (simulated page)', () => {
      ...
      let html = '<!DOCTYPE html><html><body><table class="vf-table vf-tor forumline forum"><tbody>';
      for (let i = 0; i < 50; i++) {
        html += `
  ...
  describe('Topic Details Parsing', () => {
    it('should parse topic details from HTML', () => {
      ...
      const seeders = parseInt($('.seeders').text().trim().replace(/[^\d]/g, ''), 10) || 0;
  ...
  describe('Deduplication Tests', () => {
    it('should detect existing torrent by topic_id', async () => {
      ...
      const notExistsResults = await testDb.select().from(torrents).where(eq(torrents.topicId, '999999'));
    ...
    it('should not insert duplicate torrents on second crawl', async () => {
      ...
      for (const torrent of secondCrawlTorrents) {
        ...
        const exists = await testDb.select().from(torrents).where(eq(torrents.topicId, torrent.topicId));
    ...
    it('should prevent duplicate entries even with same topic_id on different pages', async () => {
      ...
      const exists = await testDb.select().from(torrents).where(eq(torrents.topicId, '123456'));
      expect(exists.length).toBe(1);

      const notExists = await testDb.select().from(torrents).where(eq(torrents.topicId, '999999'));
