...
import { getAnnotationByUserAndTorrent, upsertAnnotation, deleteAnnotation } from '@/lib/db/queries';
...
export async function PUT(
  ...
  try {
    ...
    const result = await upsertAnnotation({
      ...
      annotation,
      readStatus,
      startedAt: readStatus === 'reading' && !existing?.started_at ? new Date().toISOString() : existing?.started_at || undefined,
      completedAt: readStatus === 'completed' && !existing?.completed_at ? new Date().toISOString() : existing?.completed_at || undefined,
    });
