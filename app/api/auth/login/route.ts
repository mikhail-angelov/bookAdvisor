...
import { getUserByEmail, upsertUser } from '@/lib/db/queries';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!emailSent) {
      ...
      console.log('[AUTH] DEV MODE - Magic link:', `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/auth/verify?token=${token}`);
    }
