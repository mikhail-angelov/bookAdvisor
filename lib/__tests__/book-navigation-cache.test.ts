...
describe('bookNavigationCache', () => {
  ...
  it('returns previous and next ids for a cached snapshot', () => {
    const key = bookNavigationCache.save('books', ['book-1', 'book-2', 'book-3']);
  ...
  it('deduplicates ids and handles edges', () => {
    const key = bookNavigationCache.save('recommendations', ['book-1', 'book-1', 'book-2']);
  ...
  it('returns null when snapshot or book is missing', () => {
    ...
    const key = bookNavigationCache.save('books', ['book-1']);
